...
class DCMotor(Node):
    def __init__(self):
        ...
        # Server for initiating all nodes
        self.simulation_running = False
        # Set Server Callback
        self.srv = self.create_service(InitiateProcess, 
            'EnableProcess', self.simulation_service_callback)

        # Declare parameters
        # System sample time in seconds
        self.declare_parameter('sample_time', 0.001)
        # System gain K
        self.declare_parameter('sys_gain_K', 4.0)
        # System time constant Tau
        self.declare_parameter('sys_tau_T', 0.9)
        ...
        self.motor_input_sub = self.create_subscription(Float32, 'RCGmotor_input_u', self.input_callback,10)
        self.motor_speed_pub = self.create_publisher(Float32, 'RCGmotor_speed_y', 10)
